...
function mapFrontendItineraryToDb(frontendSegment: any, trfId: string) {
  return {
    trf_id: trfId,
    segment_date: frontendSegment.date ? formatISO(frontendSegment.date, { representation: 'date' }) : null,
    ...
    breakfast: frontendSegment.breakfast || 0,
    lunch: frontendSegment.lunch || 0,
    dinner: frontendSegment.dinner || 0,
    supper: frontendSegment.supper || 0,
    refreshment: frontendSegment.refreshment || 0,
  };
}
...
}).optional().default({}); // Make the entire object optional with default empty object
...
const itinerarySegmentSchemaDb = z.object({
  id: z.string().optional(), date: z.coerce.date().nullable(), day: z.string().min(1), from: z.string().min(1), to: z.string().min(1), 
  etd: z.string().regex(timeRegex).optional().nullable().or(z.literal("")), eta: z.string().regex(timeRegex).optional().nullable().or(z.literal("")), 
  flightNumber: z.string().optional(), flightClass: z.string().optional(), remarks: z.string().optional().nullable(),
});
const mealProvisionSchemaDb = z.object({
  dateFromTo: z.string().min(1), breakfast: z.coerce.number().int().default(0), lunch: z.coerce.number().int().default(0), 
...
const externalPartyAccommodationDetailSchemaDb = z.object({
  id: z.string().optional(), checkInDate: z.coerce.date().nullable(), checkOutDate: z.coerce.date().nullable(), placeOfStay: z.string().min(1), 
  estimatedCostPerNight: z.coerce.number().optional().nullable(), remarks: z.string().optional().nullable(),
});
const companyTransportDetailSchemaDb = z.object({
  id: z.string().optional(), date: z.coerce.date().nullable(), day: z.string().min(1), from: z.string().min(1), to: z.string().min(1), 
  btNoRequired: z.string().optional().nullable(), accommodationTypeN: z.string().optional().nullable(), address: z.string().optional().nullable(), remarks: z.string().optional().nullable(),
});
const advanceBankDetailsSchemaDb = z.object({ bankName: z.string().min(1), accountNumber: z.string().min(1) });
const advanceAmountRequestedItemSchemaDb = z.object({
  id: z.string().optional(), dateFrom: z.coerce.date().nullable(), dateTo: z.coerce.date().nullable(), 
  lh: z.coerce.number().default(0), ma: z.coerce.number().default(0), oa: z.coerce.number().default(0), 
  tr: z.coerce.number().default(0), oe: z.coerce.number().default(0), usd: z.coerce.number().optional().nullable(), remarks: z.string().optional().nullable(),
});
const domesticTrfDetailsSchema = z.object({
  purpose: z.string().min(10), itinerary: z.array(itinerarySegmentSchemaDb).min(1), mealProvision: mealProvisionSchemaDb, 
  accommodationDetails: z.array(accommodationDetailSchemaDb).optional().default([]), companyTransportDetails: z.array(companyTransportDetailSchemaDb).optional().default([]),
});
const overseasTrfDetailsSchema = z.object({
  purpose: z.string().min(10), itinerary: z.array(itinerarySegmentSchemaDb).min(1), advanceBankDetails: advanceBankDetailsSchemaDb, 
  advanceAmountRequested: z.array(advanceAmountRequestedItemSchemaDb).min(1),
});
const externalPartiesTrfDetailsSchema = z.object({
  purpose: z.string().min(10), itinerary: z.array(itinerarySegmentSchemaDb).min(1), 
  accommodationDetails: z.array(externalPartyAccommodationDetailSchemaDb).optional().default([]), mealProvision: mealProvisionSchemaDb,
});
const baseTrfSchema = z.object({
  additionalComments: z.string().optional().nullable(), confirmPolicy: z.boolean().refine(val => val === true), 
  confirmManagerApproval: z.boolean().refine(val => val === true), confirmTermsAndConditions: z.boolean().optional(), estimatedCost: z.number().optional().default(0),
});
...
export async function GET(request: NextRequest, { params }: { params: { trfId: string } }) {
  ...
  if (!sql) {
    console.error("API_TRF_TRFID_GET_CRITICAL_ERROR (PostgreSQL): SQL client is not initialized.");
  ...
  try {
    ...
    let mainTrfData;
    try {
      const result = await sql`SELECT * FROM travel_requests WHERE id = ${trfId}`;
      [mainTrfData] = result;
      if (!mainTrfData) {
      ...
      console.error(`API_TRF_TRFID_GET_DB_ERROR (PostgreSQL): Error fetching main TRF data:`, dbError);
    ...
    try {
      ...
      itinerary = await sql`
        ...
        WHERE trf_id = ${trfId}
      `;
      console.log(`API_TRF_TRFID_GET (PostgreSQL): Fetched ${itinerary.length} itinerary segments.`);
      
      // Log the first itinerary segment to debug
      if (itinerary.length > 0) {
        console.log(`API_TRF_TRFID_GET (PostgreSQL): First itinerary segment fields:`, Object.keys(itinerary[0]).join(', '));
      }
    } catch (dbError: any) {
      console.error(`API_TRF_TRFID_GET_DB_ERROR (PostgreSQL): Error fetching itinerary:`, dbError);
      // Continue execution but with empty itinerary
    }
    
    try {
      approvalWorkflow = await sql`SELECT step_role as role, step_name as name, status, step_date as date, comments FROM trf_approval_steps WHERE trf_id = ${trfId} ORDER BY created_at ASC`;
      console.log(`API_TRF_TRFID_GET (PostgreSQL): Fetched ${approvalWorkflow.length} approval steps.`);
    } catch (dbError: any) {
      console.error(`API_TRF_TRFID_GET_DB_ERROR (PostgreSQL): Error fetching approval workflow:`, dbError);
    ...
    let trfData: any = {
      ...
      submittedAt: mainTrfData.submitted_at ? formatISO(new Date(mainTrfData.submitted_at)) : undefined,
      createdAt: mainTrfData.created_at ? formatISO(new Date(mainTrfData.created_at)) : undefined,
      updatedAt: mainTrfData.updated_at ? formatISO(new Date(mainTrfData.updated_at)) : undefined,
    ...
    if (mainTrfData.travel_type === "Domestic") {
      ...
      try {
        ...
        if (tableExists[0]?.exists) {
          mealProvisionData = await sql`
            ...
            WHERE trf_id = ${trfId}
          `;
          console.log(`API_TRF_TRFID_GET (PostgreSQL): Fetched meal provision data from trf_meal_provisions table.`);
        } else {
          ...
          mealProvisionData = await sql`
            SELECT 
              ...
              COALESCE(SUM(breakfast::int), 0) as breakfast,
              COALESCE(SUM(lunch::int), 0) as lunch,
              COALESCE(SUM(dinner::int), 0) as dinner,
              COALESCE(SUM(supper::int), 0) as supper,
              COALESCE(SUM(refreshment::int), 0) as refreshment
            FROM trf_itinerary_segments 
            WHERE trf_id = ${trfId}
          `;
          console.log(`API_TRF_TRFID_GET (PostgreSQL): trf_meal_provisions table does not exist. Fetched meal provision data from itinerary segments as fallback.`);
        }
      } catch (dbError: any) {
        console.error(`API_TRF_TRFID_GET_DB_ERROR (PostgreSQL): Error fetching meal provision data:`, dbError);
        // Continue execution with empty meal provision data
      }
      const [mealProvision] = mealProvisionData.length > 0 ? mealProvisionData : [{
        dateFromTo: "", 
        breakfast: 0, 
        lunch: 0, 
        dinner: 0, 
      ...
      try {
        ...
        if (tableExists[0]?.exists) {
          ...
          console.log(`API_TRF_TRFID_GET (PostgreSQL): trf_company_transport_details table does not exist. Using empty array.`);
          ...
          if (mainTrfData.travel_type === "Domestic") {
            ...
            console.log(`API_TRF_TRFID_GET (PostgreSQL): Generated ${companyTransportDetails.length} company transport details from itinerary.`);
          }
        }
      } catch (error) {
        console.warn("API_TRF_TRFID_GET_ERROR (PostgreSQL): Error with company transport details:", error);
      }
    ...
    } else if (mainTrfData.travel_type === "Overseas" || mainTrfData.travel_type === "Home Leave Passage") {
      // Handle overseas travel details
      console.log(`API_TRF_TRFID_GET (PostgreSQL): Processing ${mainTrfData.travel_type} travel details.`);
      ...
      try {
        ...
        if (bankTableExists[0]?.exists) {
          ...
          if (bankDetailsResult && bankDetailsResult.length > 0) {
            ...
            console.log(`API_TRF_TRFID_GET (PostgreSQL): Fetched advance bank details.`);
          } else {
            console.log(`API_TRF_TRFID_GET (PostgreSQL): No advance bank details found in database.`);
            
            // Try to extract from additional_data JSON if available
            if (mainTrfData.additional_data) {
              try {
                const additionalData = typeof mainTrfData.additional_data === 'string' 
                ...
                if (additionalData && additionalData.advanceBankDetails) {
                  advanceBankDetails = {
                    ...
                    accountNumber: additionalData.advanceBankDetails.accountNumber || "",
                    accountName: additionalData.advanceBankDetails.accountName || "",
                    swiftCode: additionalData.advanceBankDetails.swiftCode || "",
                    iban: additionalData.advanceBankDetails.iban || "",
                    branchAddress: additionalData.advanceBankDetails.branchAddress || "",
                    currency: additionalData.advanceBankDetails.currency || "",
                    amount: additionalData.advanceBankDetails.amount || null
                  };
                  console.log(`API_TRF_TRFID_GET (PostgreSQL): Extracted bank details from additional_data.`);
                }
              } catch (e) {
                console.error(`API_TRF_TRFID_GET_ERROR (PostgreSQL): Failed to parse additional_data JSON for bank details:`, e);
              }
            }
          }
        }
      } catch (error) {
        console.error(`API_TRF_TRFID_GET_ERROR (PostgreSQL): Error fetching advance bank details:`, error);
      }
      ...
      try {
        ...
        if (tableExists[0]?.exists) {
          ...
          if (advanceAmountResult && advanceAmountResult.length > 0) {
            ...
            console.log(`API_TRF_TRFID_GET (PostgreSQL): Fetched ${advanceAmountRequested.length} advance amount requested items.`);
          } else {
            console.log(`API_TRF_TRFID_GET (PostgreSQL): No advance amount requested items found in database.`);
          }
        } else {
          console.log(`API_TRF_TRFID_GET (PostgreSQL): trf_advance_amount_requested_items table does not exist.`);
          ...
          if (mainTrfData.additional_data) {
            try {
              ...
              if (additionalData && additionalData.advanceAmountRequested && Array.isArray(additionalData.advanceAmountRequested)) {
                advanceAmountRequested = additionalData.advanceAmountRequested.map((item: any) => ({
                  id: item.id || "",
                  dateFrom: item.dateFrom ? safeParseISO(item.dateFrom) : null,
                ...
                console.log(`API_TRF_TRFID_GET (PostgreSQL): Extracted ${advanceAmountRequested.length} advance amount requested items from additional_data.`);
              }
            } catch (e) {
              console.error(`API_TRF_TRFID_GET_ERROR (PostgreSQL): Failed to parse additional_data JSON for advance amount:`, e);
            }
          }
          
          // Try to extract from additional_comments as a fallback
          if (advanceAmountRequested.length === 0 && mainTrfData.additional_comments) {
            try {
              const comments = mainTrfData.additional_comments;
              if (comments.includes('Advance Amount Requested:')) {
                console.log(`API_TRF_TRFID_GET (PostgreSQL): Found advance amount data in additional_comments.`);
                // Try to parse the comments for advance amount data
                const advanceSection = comments.split('Advance Amount Requested:')[1];
                if (advanceSection) {
                  const lines = advanceSection.split('\n').filter((line: string) => line.trim().length > 0);
                  const parsedItems = [];
                  ...
                  for (let i = 0; i < lines.length; i++) {
                    ...
                    if (line.includes('Date:') || line.includes('LH:')) {
                      ...
                      const dateMatch = line.match(/Date:\s*([^,]+)\s*to\s*([^,]+)/);
                      ...
                      parsedItems.push({
                        id: `parsed-${i}`,
                        dateFrom: dateMatch ? safeParseISO(dateMatch[1].trim()) : null,
                        dateTo: dateMatch ? safeParseISO(dateMatch[2].trim()) : null,
                  ...
                  if (parsedItems.length > 0) {
                    ...
                    console.log(`API_TRF_TRFID_GET (PostgreSQL): Parsed ${advanceAmountRequested.length} advance amount items from additional_comments.`);
                  }
                }
              }
            } catch (e) {
              console.error(`API_TRF_TRFID_GET_ERROR (PostgreSQL): Failed to parse advance amount data from comments:`, e);
            }
          }
        }
        ...
        if (advanceAmountRequested.length === 0) {
          console.log(`API_TRF_TRFID_GET (PostgreSQL): No advance amount data found. Creating placeholder.`);
        ...
        console.error(`API_TRF_TRFID_GET_ERROR (PostgreSQL): Error fetching advance amount requested items:`, error);
      ...
      try {
        accommodationDetailsResult = await sql`
          ...
          WHERE trf_id = ${trfId}
        `;
        console.log(`API_TRF_TRFID_GET (PostgreSQL): Fetched ${accommodationDetailsResult.length} accommodation details for external parties.`);
      } catch (dbError: any) {
        console.error(`API_TRF_TRFID_GET_DB_ERROR (PostgreSQL): Error fetching accommodation details for external parties:`, dbError);
      ...
      trfData.externalPartyRequestorInfo = { 
        ...
        externalRefToAuthorityLetter: mainTrfData.external_ref_to_authority_letter || '', 
    ...
    console.log(`API_TRF_TRFID_GET (PostgreSQL): TRF ${trfId} data fully assembled.`);
    return NextResponse.json({ trf: trfData as TravelRequestForm });
  } catch (error: any) {
    console.error(`API_TRF_TRFID_GET_ERROR (PostgreSQL) for TRF ${trfId}:`, error.message, error.stack);
    
    // Check if the error is related to a missing table or column
    const errorMessage = error.message || '';
    if (errorMessage.includes('does not exist') || errorMessage.includes('42P01')) {
      // Database schema issue
      console.error(`API_TRF_TRFID_GET_SCHEMA_ERROR (PostgreSQL): Schema mismatch detected in the database`);
      return NextResponse.json({ 
        error: `Database schema issue detected. Please contact the administrator.`,
        details: {
          message: 'The database schema does not match the expected structure.',
          technicalDetails: errorMessage.substring(0, 200) // Limit the error message length
        }
      }, { status: 500 });
    } else if (errorMessage.includes('dateString.split') || errorMessage.includes('is not a function')) {
      // Date parsing error
      console.error(`API_TRF_TRFID_GET_DATE_ERROR (PostgreSQL): Date parsing error`);
      return NextResponse.json({ 
...
export async function PUT(request: NextRequest, { params }: { params: { trfId: string } }) {
  ...
  if (!sql) {
    console.error("API_TRF_TRFID_PUT_CRITICAL_ERROR (PostgreSQL): SQL client is not initialized.");
    return NextResponse.json({
  ...
  try {
    ...
    rawBody.requestorInfo.requestorName = rawBody.requestorName || rawBody.requestorInfo.requestorName || '';
    rawBody.requestorInfo.staffId = rawBody.staffId || rawBody.requestorInfo.staffId || null;
    rawBody.requestorInfo.department = rawBody.department || rawBody.requestorInfo.department || null;
    rawBody.requestorInfo.position = rawBody.position || rawBody.requestorInfo.position || null;
    rawBody.requestorInfo.costCenter = rawBody.costCenter || rawBody.requestorInfo.costCenter || null;
    rawBody.requestorInfo.telEmail = rawBody.telEmail || rawBody.requestorInfo.telEmail || null;
    rawBody.requestorInfo.email = rawBody.email || rawBody.requestorInfo.email || '';

    // Ensure externalPartyRequestorInfo is always an object
    rawBody.externalPartyRequestorInfo = rawBody.externalPartyRequestorInfo || {};
    // Move top-level external party requestor properties into externalPartyRequestorInfo
    rawBody.externalPartyRequestorInfo.externalFullName = rawBody.externalFullName || rawBody.externalPartyRequestorInfo.externalFullName || '';
    rawBody.externalPartyRequestorInfo.externalOrganization = rawBody.externalOrganization || rawBody.externalPartyRequestorInfo.externalOrganization || '';
    rawBody.externalPartyRequestorInfo.externalRefToAuthorityLetter = rawBody.externalRefToAuthorityLetter || rawBody.externalPartyRequestorInfo.externalRefToAuthorityLetter || '';
    rawBody.externalPartyRequestorInfo.externalCostCenter = rawBody.externalCostCenter || rawBody.externalPartyRequestorInfo.externalCostCenter || '';
    ...
    if (rawBody.travelType === "Domestic") {
      ...
      rawBody.domesticTravelDetails.purpose = rawBody.purpose || rawBody.domesticTravelDetails.purpose || "";
      rawBody.domesticTravelDetails.itinerary = rawBody.itinerary || rawBody.domesticTravelDetails.itinerary || [];
      rawBody.domesticTravelDetails.mealProvision = rawBody.mealProvision || rawBody.domesticTravelDetails.mealProvision || {};
      rawBody.domesticTravelDetails.accommodationDetails = rawBody.accommodationDetails || rawBody.domesticTravelDetails.accommodationDetails || [];
      rawBody.domesticTravelDetails.companyTransportDetails = rawBody.companyTransportDetails || rawBody.domesticTravelDetails.companyTransportDetails || [];
    } else if (rawBody.travelType === "Overseas" || rawBody.travelType === "Home Leave Passage") {
      rawBody.overseasTravelDetails = rawBody.overseasTravelDetails || {};
      rawBody.overseasTravelDetails.purpose = rawBody.purpose || rawBody.overseasTravelDetails.purpose || "";
      rawBody.overseasTravelDetails.itinerary = rawBody.itinerary || rawBody.overseasTravelDetails.itinerary || [];
      rawBody.overseasTravelDetails.advanceBankDetails = rawBody.advanceBankDetails || rawBody.overseasTravelDetails.advanceBankDetails || {};
      rawBody.overseasTravelDetails.advanceAmountRequested = rawBody.advanceAmountRequested || rawBody.overseasTravelDetails.advanceAmountRequested || [];
    } else if (rawBody.travelType === "External Parties") {
      rawBody.externalPartiesTravelDetails = rawBody.externalPartiesTravelDetails || {};
      rawBody.externalPartiesTravelDetails.purpose = rawBody.purpose || rawBody.externalPartiesTravelDetails.purpose || "";
      rawBody.externalPartiesTravelDetails.itinerary = rawBody.itinerary || rawBody.externalPartiesTravelDetails.itinerary || [];
      rawBody.externalPartiesTravelDetails.accommodationDetails = rawBody.accommodationDetails || rawBody.externalPartiesTravelDetails.accommodationDetails || [];
      rawBody.externalPartiesTravelDetails.mealProvision = rawBody.mealProvision || rawBody.externalPartiesTravelDetails.mealProvision || {};
    }
    ...
    if (rawBody.purpose && rawBody.travelType) {
      ...
      } else if ((rawBody.travelType === "Overseas" || rawBody.travelType === "Home Leave Passage") && rawBody.overseasTravelDetails) {
        rawBody.overseasTravelDetails.purpose = rawBody.purpose;
      } else if (rawBody.travelType === "External Parties" && rawBody.externalPartiesTravelDetails) {
    ...
    if (rawBody.advanceBankDetails && (rawBody.travelType === "Overseas" || rawBody.travelType === "Home Leave Passage") && rawBody.overseasTravelDetails) {
      rawBody.overseasTravelDetails.advanceBankDetails = rawBody.advanceBankDetails;
    }

    // Move top-level advanceAmountRequested to appropriate nested detail if not already there
    if (rawBody.advanceAmountRequested && (rawBody.travelType === "Overseas" || rawBody.travelType === "Home Leave Passage") && rawBody.overseasTravelDetails) {
      rawBody.overseasTravelDetails.advanceAmountRequested = rawBody.advanceAmountRequested;
    }

    // Move top-level mealProvision to appropriate nested detail if not already there
    if (rawBody.mealProvision && (rawBody.travelType === "Domestic" || rawBody.travelType === "External Parties")) {
      if (rawBody.travelType === "Domestic" && rawBody.domesticTravelDetails) {
        rawBody.domesticTravelDetails.mealProvision = rawBody.mealProvision;
      } else if (rawBody.travelType === "External Parties" && rawBody.externalPartiesTravelDetails) {
        rawBody.externalPartiesTravelDetails.mealProvision = rawBody.mealProvision;
      }
    }

    // Move top-level accommodationDetails to appropriate nested detail if not already there
    if (rawBody.accommodationDetails && (rawBody.travelType === "Domestic" || rawBody.travelType === "External Parties")) {
      if (rawBody.travelType === "Domestic" && rawBody.domesticTravelDetails) {
        rawBody.domesticTravelDetails.accommodationDetails = rawBody.accommodationDetails;
      } else if (rawBody.travelType === "External Parties" && rawBody.externalPartiesTravelDetails) {
        rawBody.externalPartiesTravelDetails.accommodationDetails = rawBody.accommodationDetails;
      }
    }

    // Move top-level companyTransportDetails to appropriate nested detail if not already there
    if (rawBody.companyTransportDetails && rawBody.travelType === "Domestic" && rawBody.domesticTravelDetails) {
      rawBody.domesticTravelDetails.companyTransportDetails = rawBody.companyTransportDetails;
    }
    
    const validationResult = trfSubmissionSchema.safeParse(rawBody);

    if (!validationResult.success) {
      console.error("API_TRF_TRFID_PUT_VALIDATION_ERROR (PostgreSQL):", validationResult.error.flatten());
      return NextResponse.json({ 
  ...
  try {
    const [existingTrf] = await sql`SELECT status FROM travel_requests WHERE id = ${trfId}`;
    if (!existingTrf) {
      return NextResponse.json({ error: "TRF not found for update." }, { status: 404 });
    }
    const editableStatuses = ["Pending Department Focal", "Rejected", "Draft"];
    if (!editableStatuses.includes(existingTrf.status as string)) {
      return NextResponse.json({ error: `TRF with status ${existingTrf.status} cannot be edited.` }, { status: 403 });
    }

    let requestorNameVal: string | null = validatedData.requestorInfo?.requestorName || null;
    let staffIdVal: string | null = validatedData.requestorInfo?.staffId || null;
    let departmentVal: string | null = validatedData.requestorInfo?.department || null;
    let positionVal: string | null = validatedData.requestorInfo?.position || null;
    let costCenterVal: string | null = validatedData.requestorInfo?.costCenter || null;
    let telEmailVal: string | null = validatedData.requestorInfo?.telEmail || null;
    let emailVal: string | null = validatedData.requestorInfo?.email || null;
    let externalFullNameVal: string | null = validatedData.externalPartyRequestorInfo?.externalFullName || null;
    let externalOrganizationVal: string | null = validatedData.externalPartyRequestorInfo?.externalOrganization || null;
    let externalRefToAuthorityLetterVal: string | null = validatedData.externalPartyRequestorInfo?.externalRefToAuthorityLetter || null;
    let externalCostCenterVal: string | null = validatedData.externalPartyRequestorInfo?.externalCostCenter || null;
    ...
    await sql.begin(async tx => {
      await tx`
        ...
        WHERE id = ${trfId}
      `;
      console.log("API_TRF_TRFID_PUT (PostgreSQL): Updated travel_requests for ID:", trfId);
      ...
      if (tableExists[0]?.exists) {
        console.log("API_TRF_TRFID_PUT (PostgreSQL): trf_company_transport_details table exists. Deleting records.");
        await tx`DELETE FROM trf_company_transport_details WHERE trf_id = ${trfId}`;
      } else {
        console.log("API_TRF_TRFID_PUT (PostgreSQL): trf_company_transport_details table does not exist. Skipping delete.");
      }
      ...
      `;
      console.log("API_TRF_TRFID_PUT (PostgreSQL): Handled related tables deletions and added 'Edited' step for ID:", trfId);
      ...
      if (itineraryToSave.length > 0) {
        const itineraryInserts = itineraryToSave.map(seg => mapFrontendItineraryToDb(seg, trfId));
        await tx`INSERT INTO trf_itinerary_segments ${tx(itineraryInserts, 'trf_id', 'segment_date', 'day_of_week', 'from_location', 'to_location', 'departure_time', 'arrival_time', 'flight_number', 'flight_class', 'remarks', 'breakfast', 'lunch', 'dinner', 'supper', 'refreshment')}`;
      }
      ...
      if (validatedData.travelType === "Domestic") {
        ...
        if (details.accommodationDetails && details.accommodationDetails.length > 0) {
          const accomInserts = details.accommodationDetails.map((acc: any) => ({
            trf_id: trfId, accommodation_type: acc.accommodationType, other_type_description: acc.otherTypeDescription || null,
        ...
        if (details.companyTransportDetails && details.companyTransportDetails.length > 0) {
          const transportInserts = details.companyTransportDetails.map((trans: any) => ({
            trf_id: trfId, 
            transport_date: trans.date ? formatISO(trans.date, { representation: 'date' }) : null, 
          ...
          if (tableExists[0]?.exists) {
            console.log("API_TRF_TRFID_PUT (PostgreSQL): trf_company_transport_details table exists. Inserting records.");
            try {
              ...
              console.log("API_TRF_TRFID_PUT (PostgreSQL): Successfully inserted transport details.");
            } catch (error: any) {
              console.error("API_TRF_TRFID_PUT_ERROR (PostgreSQL): Error inserting transport details:", error);
              // Continue with the rest of the update process
            }
          } else {
            console.log("API_TRF_TRFID_PUT (PostgreSQL): trf_company_transport_details table does not exist. Skipping insert.");
          }
        }
        ...
        if (details.mealProvision) {
          await tx`INSERT INTO trf_meal_provisions (trf_id, date_from_to, breakfast, lunch, dinner, supper, refreshment) VALUES (${trfId}, ${details.mealProvision.dateFromTo}, ${Number(details.mealProvision.breakfast)}, ${Number(details.mealProvision.lunch)}, ${Number(details.mealProvision.dinner)}, ${Number(details.mealProvision.supper)}, ${Number(details.mealProvision.refreshment)})`;
        }
      } else if (validatedData.travelType === "Overseas" || validatedData.travelType === "Home Leave Passage") {
        ...
        if (details.advanceAmountRequested && details.advanceAmountRequested.length > 0) {
          const advanceAmountInserts = details.advanceAmountRequested.map((item: any) => ({
            trf_id: trfId,
      ...
      console.log("API_TRF_TRFID_PUT (PostgreSQL): Transaction committed for update of ID:", trfId);
    });
    ...
    const [updatedFullTrf] = await sql`SELECT * FROM travel_requests WHERE id = ${trfId}`;

    return NextResponse.json({ message: 'TRF updated successfully!', trf: { ...updatedFullTrf, id: trfId, status: 'Pending Department Focal' } }); // Simplified return

  } catch (error: any) {
    console.error(`API_TRF_TRFID_PUT_ERROR (PostgreSQL) for TRF ${trfId}:`, error.message, error.stack);
    return NextResponse.json({ error: 'Failed to update TRF.', details: error.message }, { status: 500 });
  }
}