...
import OverseasTravelDetailsForm from '@/components/trf/OverseasTravelDetailsForm';
...
const STEPS = ["Requestor Information", "Overseas Travel Details", "Approval & Submission"];
...
const parseDatesInOverseasTSRData = (data: any): Partial<TravelRequestForm> => {
  if (!data) return {};
  const parsed = { ...data };
  ...
  if (parsed.overseasTravelDetails) {
...
export default function OverseasFormContent() {
    ...
    if (isEditMode && editId) {
      ...
      const fetchTrfForEdit = async () => {
        try {
          ...
          const fetchedTsr = parseDatesInOverseasTSRData(result.trf) as TravelRequestForm;

          if (fetchedTsr && fetchedTsr.travelType === 'Overseas') {
            ...
            if (fetchedTsr.overseasTravelDetails) {
            ...
            throw new Error(`TSR ${editId} is not an Overseas TSR or data is invalid.`);
          }
        } catch (err: any) {
  ...
  const handleFinalSubmit = async (data: ApprovalSubmissionData) => {
    ...
    const finalTSRData = {
      ...
      estimatedCost: 2000, // Mock estimated cost
    };
    ...
    try {
      ...
      const result = await response.json();
      toast({ 
        title: `Overseas TSR ${isEditMode ? 'Updated' : 'Submitted'}!`, 
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
              <CardTitle className="flex items-center gap-2 text-2xl">
                ...
                {isEditMode ? 'Edit Overseas Travel Request' : 'New Overseas Travel Request'}
              </CardTitle>
              <CardDescription>Follow the steps to complete your overseas travel & service request.</CardDescription>
            </div>
