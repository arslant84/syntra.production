...
const parseDatesInOverseasTRFData = (data: any): Partial<TravelRequestForm> => {
  if (!data) return {};
  const parsed = { ...data };
...
export default function NewOverseasTRFPage() {
    ...
    if (isEditMode && editId) {
      ...
      const fetchTrfForEdit = async () => {
        try {
          ...
          const fetchedTrf = parseDatesInOverseasTRFData(result.trf) as TravelRequestForm;

          if (fetchedTrf && fetchedTrf.travelType === 'Overseas') {
            ...
            if (fetchedTrf.overseasTravelDetails) {
            ...
            throw new Error(`TRF ${editId} is not an Overseas TRF or data is invalid.`);
          }
        } catch (err: any) {
  ...
  const handleFinalSubmit = async (data: ApprovalSubmissionData) => {
    ...
    try {
      ...
      const result = await response.json();
      toast({ 
        title: `Overseas TRF ${isEditMode ? 'Updated' : 'Submitted'}!`, 
        ...
        <CardHeader className="bg-muted/30">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <CardDescription>Follow the steps to complete your overseas travel request form.</CardDescription>
            </div>
