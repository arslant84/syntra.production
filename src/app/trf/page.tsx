  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <Link href="/trf/new" passHref><Button><PlusCircle className="mr-2 h-5 w-5" /> Create New TRF</Button></Link>
      </div>

      <Card>
        <CardHeader><CardTitle>Filter & Search TRFs</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            <Input placeholder="Search by ID, Requestor, Purpose..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="lg:col-span-2"/>
            <Select value={statusFilter} onValueChange={setStatusFilter}>
                <SelectTrigger><ListFilter className="mr-2 h-4 w-4" /><SelectValue placeholder="Filter by Status" /></SelectTrigger>
                <SelectContent><SelectItem value={ALL_STATUSES_VALUE}>All Statuses</SelectItem>{trfStatusesList.map(status => (<SelectItem key={status} value={status}>{status}</SelectItem>))}</SelectContent>
            </Select>
            <Select value={travelTypeFilter} onValueChange={setTravelTypeFilter}>
                <SelectTrigger><ListFilter className="mr-2 h-4 w-4" /><SelectValue placeholder="Filter by Travel Type" /></SelectTrigger>
                <SelectContent><SelectItem value={ALL_TRAVEL_TYPES_VALUE}>All Travel Types</SelectItem>{travelTypesList.map(type => (<SelectItem key={type} value={type}>{type}</SelectItem>))}</SelectContent>
            </Select>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading && trfs.length === 0 ? (
             <div className="flex items-center justify-center h-40"><Loader2 className="h-8 w-8 animate-spin text-primary" /><p className="ml-2 text-muted-foreground">Loading TRFs...</p></div>
          ) : error ? (
             <div className="text-center py-8"><AlertTriangle className="mx-auto h-12 w-12 text-destructive" /><h3 className="mt-2 text-lg font-medium text-destructive">Error Loading TRFs</h3><p className="mt-1 text-sm text-muted-foreground">{error}</p><Button onClick={() => fetchTrfs(1)} className="mt-4">Try Again</Button></div>
          ) : trfs.length > 0 ? (
            <>
            <div className="overflow-x-auto">
              <Table>
                <TableHeader><TableRow><SortableHeader columnKey="id" label="TRF ID" /><SortableHeader columnKey="requestorName" label="Requestor" /><SortableHeader columnKey="travelType" label="Type" /><SortableHeader columnKey="purpose" label="Purpose" /><SortableHeader columnKey="status" label="Status" /><SortableHeader columnKey="submitted_at" label="Submitted" /><TableHead>Action</TableHead></TableRow></TableHeader>
                <TableBody>
                  {trfs.map((trf) => (
                    <TableRow key={trf.id}>
                      ...
                      <TableCell><Badge variant={getStatusBadgeVariant(trf.status)} className={trf.status === "Approved" ? "bg-green-600 text-white" : ""}>{trf.status}</Badge></TableCell>
                      <TableCell>{trf.submittedAt && isValid(parseISO(trf.submittedAt)) ? format(parseISO(trf.submittedAt), 'PPP') : 'N/A'}</TableCell>
                      <TableCell><Button variant="outline" size="sm" asChild><Link href={`/trf/view/${trf.id}`} className="flex items-center"><Eye className="mr-1.5 h-4 w-4" /> View</Link></Button></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
            ...
            <div className="flex flex-col items-center justify-center h-64 border-2 border-dashed rounded-lg"><FileText className="w-16 h-16 text-muted-foreground mb-4" /><p className="text-muted-foreground">{hasActiveFilters ? "No TRFs found matching your criteria." : "No TRFs found."}</p><p className="text-sm text-muted-foreground">Click "Create New TRF" to get started.</p>{hasActiveFilters && <Button variant="link" onClick={handleClearFilters} className="mt-2">Clear Filters</Button>}</div>
          )}
        </CardContent>
      </Card>
    </div>
