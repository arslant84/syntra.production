...
type SidebarContextType = {
  state: "expanded" | "collapsed" // Visual state
  open: boolean // Visual open state (isPinned || isHoverExpanded)
  isPinned: boolean
  setIsPinned: (pinned: boolean | ((current: boolean) => boolean)) => void
  setIsHoverExpanded: (hoverExpanded: boolean) => void
  openMobile: boolean
...
const SidebarProvider = React.forwardRef<
  ...
  React.ComponentProps<"div"> & {
    defaultOpen?: boolean // This now controls default "pinned" state
  ...
  (
    ...
    const toggleSidebar = React.useCallback(() => {
      if (isMobile) {
        ...
        setIsHoverExpanded(false) // Ensure hover state is reset on explicit toggle
      }
...
>(
  (
    ...
    const { isMobile, state, openMobile, setOpenMobile, setIsHoverExpanded, isPinned } = sidebarContext;

    const handleMouseEnter = () => {
      if (!isMobile && collapsible === 'icon' && !isPinned) {
...
>(({ className, ...props }, ref) => {
  const sidebarContext = useSidebar()
  if (!sidebarContext) return null;
  const { toggleSidebar } = sidebarContext; // This toggles the "pinned" state
...
>(
  (
    ...
    if (!sidebarContext) return <Comp ref={ref} className={cn(sidebarMenuButtonVariants({ variant, size }), className)} {...props} />;
    
    const { isMobile, state: visualDataState } = sidebarContext; // Use visualDataState for tooltip logic
    ...
    return (
      <Tooltip>
        ...
        <TooltipContent
          side="right"
          align="center"
          hidden={visualDataState !== "collapsed" || isMobile} // Use visualDataState
          {...tooltip}
        />
      </Tooltip>
    )
