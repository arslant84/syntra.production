...
export class TSRAutoGenerationService {
  ...
  private static async generateTransportRequestFromDb(completeTsrData: any, tsrData: TSRData, userId: string): Promise<string | null> {
    try {
      ...
      // Create transport request without notifications
      const transportRequest = await TransportService.createTransportRequestWithoutNotifications(transportRequestData);
      ...
      // Send auto-generated request notification
      try {
        ...
        await EnhancedWorkflowNotificationService.sendAutoGeneratedRequestNotification({
          ...
          requestorId: userId,
          department: tsrData.department,
          purpose: transportRequestData.purpose,
          parentEntityType: 'tsr',
          parentEntityId: tsrData.id
        });

        console.log(`✅ TSR_AUTO_GEN: Sent auto-generated notification for transport request ${transportRequest.id}`);
      } catch (notificationError) {
        console.error(`❌ TSR_AUTO_GEN: Failed to send auto-generated notification for transport request ${transportRequest.id}:`, notificationError);
        // Don't fail the generation due to notification errors
      }
  ...
  private static async generateAccommodationRequest(tsrData: TSRData, accommodationDetail: any, userId: string): Promise<string | null> {
    try {
      ...
      // Send auto-generated request notification for accommodation
      try {
        // Get requestor email for enhanced notifications
        const requestorInfo = await sql`SELECT email FROM users WHERE id = ${userId}`;
        const requestorEmail = requestorInfo.length > 0 ? requestorInfo[0].email : null;
        ...
        console.log(`✅ TSR_AUTO_GEN: Sent auto-generated notification for accommodation request ${accomRequestId}`);
      } catch (notificationError) {
        console.error(`❌ TSR_AUTO_GEN: Failed to send auto-generated notification for accommodation request ${accomRequestId}:`, notificationError);
        // Don't fail the generation due to notification errors
      }
